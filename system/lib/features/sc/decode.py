...
def sc_decode():
    ...
    for file in files:
        ...
        try:
            ...
            data = (
                struct.pack("4s?B", b"XCOD", use_lzham, len(swf.textures))
                + swf.xcod_writer.getvalue()
            )

            with open(
                f'{folder_export}/{current_sub_path}/{base_name.rstrip("_")}.xcod', "wb"
            ) as xc:
                xc.write(data)
            for img_index in range(len(swf.textures)):
                filename = base_name + "_" * img_index
                swf.textures[img_index].image.save(
                    f"{folder_export}/{current_sub_path}/{filename}.png"
                )
